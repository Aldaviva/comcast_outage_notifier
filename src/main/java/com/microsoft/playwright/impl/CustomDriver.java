...
public class CustomDriver extends Driver {
		...
		if (instance == null) {
			try {
				if (System.getProperty("playwright.cli.dir") != null) {
					return Driver.ensureDriverInstalled(extraEnvironmentVariables);
				}

				instance = new CustomDriver();
				instance.initialize(browsersToInstall, extraEnvironmentVariables);
	...
	@Override
	protected void initialize(final Map<String, String> env) throws Exception {
		initialize(null, env);
	}

	protected void initialize(final Collection<String> browsersToInstall, final Map<String, String> extraEnvironmentVariables) throws Exception {
